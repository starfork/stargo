...
// Fetch gets values from file cache.
// if nonexistent or expired return an empty string.
func (fc *FileCache) Fetch(ctx context.Context, keys []string) ([]any, error) {
	...
	return rc, cache.ErrMultiGetFailed
	//berror.Error(MultiGetFailed, strings.Join(keysErr, "; "))
}
...
func (fc *FileCache) Put(ctx context.Context, key string, val interface{}, timeout ...time.Duration) error {
	...
	if len(timeout) > 0 {
		tm := timeout[0]
		if tm == time.Duration(fc.EmbedExpiry) {
			item.Expired = time.Now().Add((86400 * 365 * 10) * time.Second) // ten years
		} else {
			item.Expired = time.Now().Add(tm)
		}
	}
...
func FileGetContents(filename string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not read the data from the file: %s, "+
...
func GobEncode(data interface{}) ([]byte, error) {
	...
	err := enc.Encode(data)
	if err != nil {
		return nil, fmt.Errorf("could not encode this data")
...
func GobDecode(data []byte, to *FileCacheItem) error {
	...
	err := dec.Decode(&to)
	if err != nil {
		return fmt.Errorf("could not decode this data to FileCacheItem. Make sure that the data is encoded by GOB.")
